...
router.get("/", async (req, res) => {
  try {
    ...
    await client.connect();
    const db = client.db("Hackthon");
    const collection = await db.collection("DocAccountData").aggregate().toArray();
    const AddCollection = db.collection("DocAccountData");
    ...
    if (isexist == 0) {
      ...
      userID = await fetchData.GetID(client,"Hackthon","DocAccountData",username);
